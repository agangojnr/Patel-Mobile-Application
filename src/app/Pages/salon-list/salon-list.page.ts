...
export class SalonListPage implements OnInit {
  ...
  ionViewWillEnter() {
    if (localStorage.getItem("genderFilter")) {
     ...
     // console.log("this.genderFilter.type: ", this.genderFilter.type);
       ...
       localStorage.setItem("genderFilter", "All Links");
      this.type = "All Links";
    }
  ...
  doRefresh(event) {
    // this.getUserDate();
    // console.log("this.data: ", this.api.id);
    this.api.getDataWithToken("category/" + this.api.id + "/branch").subscribe(
      (res: any) => {
        if (res.success) {
          this.data = res.data;
  ...
  genderFilterFunc() {
       let g_info: any;
         this.catgoryId = this.api.id;

        // console.log(this.catgoryId);


    if (this.type == "Investor Link") {
      this.genderFilter.for_who = 1;
    } else if (this.type == "Co-operate Link") {
  ...
  this.util.startLoad();
    this.api.getDataWithToken("filleterbycategory/branch/"+this.catgoryId+"/"+g_info).subscribe(
      (res: any) => {
...
//console.log(this.genderFilter.for_who);
